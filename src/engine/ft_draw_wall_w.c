...
void	ft_draw_w_w_d_2(t_draw_w_var v, t_ray_struct *ray_s,
...
void	ft_draw_wall_w_down(t_draw_w_var v, t_ray_struct *ray_s,
	t_ray_hit hit_info, int col)
{
	...
	while (v.half > 0)
	{
		...
		v.new_line = (ray_s->tex->west_w - v.x - 1)
			+ ray_s->tex->west_w * v.new_line;
		if (v.ydown > ray_s->rdr->wid_len)
			break ;
		if (v.new_line > ray_s->tex->west_len)
			v.new_line = ray_s->tex->west_len - 1;
		ratio = set_darkness(ray_s->tex->tex_w[v.new_line], hit_info.dark_r);
		if (v.ydown >= 0)
	...
	ft_draw_w_w_d_2(v, ray_s, pixel, shadow);
}

void	ft_d_w_w_u_2(t_draw_w_var v, t_ray_struct *ray_s,
		int pixel, float shadow)
{
	t_darken	ratio;
...
void	ft_draw_wall_w_up(t_draw_w_var v, t_ray_struct *ray_s,
	t_ray_hit hit_info, int col)
{
	t_darken	ratio;
	float		shadow;
	...
	ft_d_w_w_u_2(v, ray_s, pixel, shadow);
	v.j = v.wall_size_2;
	ft_draw_wall_w_down(v, ray_s, hit_info, col);
}
	...
	int col, t_ray_hit hit_info)
{
	...
	v.line = ray_s->tex->west_h / wall_size;
	v.x = hit_info.tex_off_y * ray_s->tex->w_r_tex_w;
	...
	v.j = v.wall_size_2;
	v.i = 0;
	ft_draw_wall_w_up(v, ray_s, hit_info, col);
}
