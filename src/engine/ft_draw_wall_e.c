...
void	ft_draw_w_e_d_2(t_draw_w_var v, t_ray_struct *ray_s,
...
void	ft_draw_w_e_d_3(t_draw_w_var v, t_ray_struct *ray_s, float shadow,
		int col)
{
	t_darken	ratio;
	int			pixel;
...
void	ft_draw_wall_e_down(t_draw_w_var v, t_ray_struct *ray_s,
	t_ray_hit hit_info, int col)
{
	...
	shadow = 1;
	pixel = col - 1;
	ft_draw_w_e_d_2(v, ray_s, pixel, shadow);
	while (v.half > 0)
	{
		v.new_line = (int)(v.line * v.j);
		v.new_line = v.x + ray_s->tex->east_w * v.new_line;
		if (v.ydown > ray_s->rdr->wid_len)
			break ;
		if (v.new_line > ray_s->tex->east_len)
			v.new_line = ray_s->tex->east_len - 1;
		ratio = set_darkness(ray_s->tex->tex_e[v.new_line], hit_info.dark_r);
		if (v.ydown >= 0)
	...
	ft_draw_w_e_d_3(v, ray_s, shadow, col);
}

void	ft_draw_wall_e_up(t_draw_w_var v, t_ray_struct *ray_s,
	t_ray_hit hit_info, int col)
{
	t_darken	ratio;
	...
	v.j = v.wall_size_2;
	ft_draw_wall_e_down(v, ray_s, hit_info, col);
}
	...
	int col, t_ray_hit hit_info)
{
	...
	v.line = ray_s->tex->east_h / wall_size;
	v.x = hit_info.tex_off_y * ray_s->tex->w_r_tex_e;
	...
	v.j = v.wall_size_2;
	v.i = 0;
	ft_draw_wall_e_up(v, ray_s, hit_info, col);
}
